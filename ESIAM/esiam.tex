...
Detailed comparisons of the URG with other methods (e.g., the functional RG, spectrum bifurcation RG etc.) can be found in Refs.~\cite{anirbanmott1,anirbanurg1}. More information on the unitary RG method is provided in Chapter 2.
...
Renormalisation to the bath interaction \(U_b\) can arise purely from itself, because of the lack of any impurity operators. We show in Section~\ref{tileRG} of Chapter 5 that this coupling is in fact marginal. 
...
\end{eqnarray}
The renormalisation in \(V\) from \(U_b\) arises through terms of \(V U_b\) and \(U_b V\) kind. We now show that this contribution vanishes entirely.

One particular scattering process in this category involves adding an electron on the impurity site:
\begin{equation}\begin{aligned}\label{VUb1}
	\sum_{k,q,\beta}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{\omega - H_D} U_b \left[\beta\beta c^\dagger_{q\beta}c_{k\beta} n_{q^\prime,\beta} + \beta\beta n_{q^\prime,\beta}c^\dagger_{q\beta}c_{k\beta} + \beta\bar\beta c^\dagger_{q\beta}c_{k\beta} n_{q^\prime,\bar\beta} + \beta\bar\beta n_{q^\prime,\bar\beta}c^\dagger_{q\beta}c_{k\beta}\right]~.
\end{aligned}\end{equation}
We set \(\beta \beta = 1 \) and \(\beta \bar\beta=-1\). For \(q^\prime \neq q\), the right hand side simplifies into the difference in densities of the two spin components of \(q^\prime\):
\begin{equation}\begin{aligned}
	\sum_{k,q,\beta}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{\omega - H_D} U_b \left[2c^\dagger_{q\beta}c_{k\beta} n_{q^\prime,\beta} - 2c^\dagger_{q\beta}c_{k\beta} n_{q^\prime,\bar\beta}\right] = \sum_{k,q,\beta}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{\omega - H_D} 4U_b \beta c^\dagger_{q\beta}c_{k\beta} s^z_{q^\prime}~.
\end{aligned}\end{equation}
In the presence of time-reversal symmetry, we expect the magnetisation \(s_{q^\prime}^z = \frac{1}{2}\sum_\beta \beta n_{q^\prime,\beta}\) to vanish in the initial state.

The case of \(q=q^\prime\) is allowed only in the second term, which leaves us with
\begin{equation}\begin{aligned}
	\sum_{k,q,\beta}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{\omega - H_D} U_b n_{q,\beta}c^\dagger_{q\beta}c_{k\beta} = \sum_{k,q,\beta}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{\omega - H_D} U_b c^\dagger_{q\beta}c_{k\beta}~.
\end{aligned}\end{equation}
We can resolve this expression into various parts depending on the intermediate energies and terminal energies:
\begin{equation}\begin{aligned}\label{Vub1}
	\sum_{k,q,\beta}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{2}\left[n_{d\bar\beta}\left(\frac{1}{\omega_1 - E_1} + \frac{1}{\omega_2 - E_1}\right) + (1 - n_{d\bar\beta})\left(\frac{1}{\omega_0 - E_0} + \frac{1}{\omega_1 - E_0}\right)\right] U_b c^\dagger_{q\beta}c_{k\beta}~.
\end{aligned}\end{equation}
 A second class of processes is generated by reversing the sequence of operations:
 \begin{equation}\begin{aligned}
	\sum_{k,q,\beta}U_b \left[\beta\beta c^\dagger_{q\beta}c_{k\beta} n_{q^\prime,\beta} + \beta\beta n_{q^\prime,\beta}c^\dagger_{q\beta}c_{k\beta} + \beta\bar\beta c^\dagger_{q\beta}c_{k\beta} n_{q^\prime,\bar\beta} + \beta\bar\beta n_{q^\prime,\bar\beta}c^\dagger_{q\beta}c_{k\beta}\right] \frac{1}{\omega - H_D} Vc^\dagger_{d\beta}c_{q\beta}~.
 \end{aligned}\end{equation}
The left hand side simplifies in the same fashion as did the right hand side in the previous class. Again considering the residual \(q=q^\prime\) contribution, we get
\begin{equation}\begin{aligned}
	\sum_{k,q,\beta}U_b n_{q^\prime,\beta}c^\dagger_{q\beta}c_{k\beta} \frac{1}{\omega - H_D} Vc^\dagger_{d\beta}c_{q\beta} = \sum_{k,q,\beta}U_b c^\dagger_{q\beta}c_{k\beta} \frac{1}{\omega - H_D} Vc^\dagger_{d\beta}c_{q\beta}~.
\end{aligned}\end{equation}
Resolving this expression into intermediate and terminal state energies gives
\begin{equation}\begin{aligned}
	\sum_{k,q,\beta}U_b c^\dagger_{q\beta}c_{k\beta} \frac{1}{2}\left[n_{d\bar\beta}\left(\frac{1}{\omega_1 - E_2} + \frac{1}{\omega_2 - E_2}\right) + (1 - n_{d\bar\beta})\left(\frac{1}{\omega_0 - E_1} + \frac{1}{\omega_1 - E_1}\right)\right] Vc^\dagger_{d\beta}c_{q\beta}~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}\label{Vub2}
	\sum_{k,q,\beta}U_b c^\dagger_{q\beta}c_{k\beta} \frac{1}{2}\left[n_{d\bar\beta}\left(\frac{1}{\omega_1 - E_0} + \frac{1}{\omega_1 - E_1}\right) + (1 - n_{d\bar\beta})\left(\frac{1}{\omega_0 - E_1} + \frac{1}{\omega_1 - E_1}\right)\right] Vc^\dagger_{d\beta}c_{q\beta}~.
\end{aligned}\end{equation}
Taken together with eq.~\ref{Vub1}, the contribution reduces to the anticommutator of \(c^\dagger+{d\beta}\) and \(c_{k\beta}\), which vanishes. A similar calculation shows that the contribution from the processes involving the destruction of the impurity electron (\(c_{d\beta}\)) also vanishes.
...
\begin{eqnarray}
	...
	\Delta J &= -\frac{n_j J\left(J + 4U_b\right)}{d_2}~,\quad &\Delta U_b = 0~,
\end{eqnarray}
...
The bath coupling \(U_b\) is marginal. The RG equations of the remaining couplings for a given quantum fluctuation scale \(\omega\) are:
\begin{align}
	\Delta U &= 4V^2 n_j\left(\frac{1}{d_1} - \frac{1}{d_0}\right) - n_j\frac{J^2}{d_2},\nonumber\\
	\Delta V &= -\frac{3n_j VJ}{8}\left(\frac{1}{d_2} + \frac{1}{d_1}\right),\label{rg-eqn}\\
...
\begin{figure}[!htb]
	\includegraphics[width=0.45\textwidth]{ESIAM/corrs_gs.pdf}
	\hspace*{\fill}
...
The entanglement measures $\varepsilon_{\text{ss}}$ and $\varepsilon_{\text{ct}}$ can be related to the impurity Greens function through the following equation (details can be found in Appendix 1 of this chapter):
\begin{eqnarray}
...
\subsection{Hysteresis and the first-order line}

As discussed above, the localisation of charge in the emergent local moment states \(\ket{1,\sigma,-}\) and \(\ket{3,\sigma,-}\) for \(r > r_{c1}\) means they do not contribute any spectral weight to the impurity spectral function. Instead, they lead to an emergent preformed/optical gap between the central Kondo resonance and the Hubbard sidebands on each side. This {\it optical gap} finally becomes the true Mott gap at \(r = r_{c2}\) when the central peak disappears.
On the other hand, upon starting from \(r > r_{c2}\) and reducing \(r\), the system is initially in the (doubly degenerate) local moment ground states, and the impurity DOS is of course gapped. In the rest of the subsection, we will show that the presence of these two transitions leads to finite temperature behaviour for the e-SIAM that is qualitatively similar to that of the Hubbard model as seen from DMFT (that was described at the beginning of this section, and can also be seen in Fig.~\ref{coexistence-schematic}).
...
\begin{eqnarray}
	Z(r,T) \simeq Z_M(r,T) + Z_I(r,T),\quad F(r,T) \simeq -k_B T \ln \left(Z_M(r,T) + Z_I(r,T)\right),\label{free energy eq}
\end{eqnarray}
where \(Z_M(r,T) = \exp{\left(-\beta \mathcal{V}\left(E_M(r,T) - TS_M(r,T)\right)\right)}\) is the partition function corresponding to the metallic state with energy \(E_M(r,T)\) and entropy \(S_M(r,T)\) per unit volume \(\mathcal{V}\), and 
\begin{equation}\begin{aligned}
Z_I = \exp{\left(-\beta \mathcal{V}\left(E_I(r,T) - TS_I(r,T)\right)\right)}~,
\end{aligned}\end{equation}
is similarly the partition function corresponding to the local moment states.

In the thermodynamic limit, only the larger of \(Z_M\) and \(Z_I\) contributes to the partition function $Z(r,T)$. Given a temperature \(T\), there exists a value \(r_c(T)\) lying in the range \(r_{c1}(T) < r_{c}(T) < r_{c2}(T)\) at which the partition functions \(Z_M(T)\) and \(Z_I(T)\) become equal: \(Z_M(r_c(T),T) = Z_I(r_c(T),T)\). This condition defines a curve \(\left\{ (T, r_c(T)) \right\} \) at which the system undergoes a first-order transition between the metallic and insulating states:
\begin{eqnarray}
	\lim_{\mathcal{V} \to \infty} F(r,T) = 
	\begin{cases}
	-k_B T \ln Z_M(r,T) \text{ if } r < r_c ~ (Z_M > Z_I)\\
	-k_B T \ln Z_I(r,T) \text{ if } r > r_c ~ (Z_M < Z_I)
	\end{cases}~.
\end{eqnarray}
...
Since the total Hamiltonian conserves the total spin \(S_\text{tot}^z\), the effective Hamiltonian separates into two sectors, \(S_\text{tot}^z = 0\) and \(|S_\text{tot}^z| = \frac{1}{2}\). Detailed calculations on obtaining this effective Hamiltonian from the RG fixed point theory are shown in Appendix 5. In order to highlight certain distinct features, we present simplified effective Hamiltonians only for the \(S_\text{tot}^z = 0\) and \(S_\text{tot}^z = \frac{1}{2}\) sectors. We first take a look at the \(S^z_\text{tot}=0\) sector:
\begin{eqnarray}
...
More indications of non-Fermi liquid behaviour at the QCP is obtained from a calculation of the impurity magnetisation and the impurity entanglement entropy (\(S_\text{EE}(d)\)) (shown in Appendix 7) for the symmetry-broken states \(\ket{S_\text{tot}^z = \sigma/2}\). This 
...
In Appendix 7, we show that the incomplete magnetisation (and hence the fractional value of \(S_\text{EE}\)) arises from the mixing of the local Fermi liquid ground-state \(\ket{\phi} = \otimes_{k <= k_F} \ket{k \uparrow}\ket{k \downarrow}\) and the gapless excitations above it, \(\ket{e}_\sigma = e^\dagger_\sigma \ket{\phi}, e \equiv \sum_{k \in FS} c^\dagger_{k\sigma}\), leading to the decay of the local Fermi liquid quasiparticles~\cite{varma2002singular}. This is captured by the modified ground-state \(\ket{-}\) and lowest-lying excited states \(\ket{+}\) at the QCP:
...
We show in Appendix 7 that it is in fact  possible to connect the scattering phase shift ($\delta^{*}$), impurity magnetisation ($m_{\text{imp}}^{z}$), effective degeneracy ($\tilde{g}$) and excess charge ($n_{\text{exc}}$) in a single relation:
\begin{eqnarray}
...
\clearpage
\section*{Appendix 1: Expressing correlation functions in terms of entanglement}
\addcontentsline{toc}{section}{\protect\numberline{}Appendix 1: Expressing correlation functions in terms of entanglement}
\label{general-ent}
...
\section*{Appendix 2: One-shot decoupling of impurity from zeroth site}
\addcontentsline{toc}{section}{\protect\numberline{}Appendix 2: One-shot decoupling of impurity from zeroth site}
...
\section*{Appendix 3: Effective Hamiltonian for the excitations within the Hubbard sidebands}
\addcontentsline{toc}{section}{\protect\numberline{}Appendix 3: Effective Hamiltonian for the excitations within the Hubbard sidebands}
...
\section*{Appendix 4: Behaviour of the Kondo scale $T_K$ close to the transition}
\addcontentsline{toc}{section}{\protect\numberline{}Appendix 4: Behaviour of the Kondo scale $T_K$ close to the transition}
\label{app-Tk}
...
\section*{Appendix 5: Effective Hamiltonian for the low-lying excitations at the critical point}
\addcontentsline{toc}{section}{\protect\numberline{}Appendix 5: Effective Hamiltonian for the low-lying excitations at the critical point}
\label{app-nfl}
...
\section*{Appendix 6: Non-Fermi liquid exponents in self-energy and two-particle correlations}
\addcontentsline{toc}{section}{\protect\numberline{}Appendix 6: Non-Fermi liquid exponents in self-energy and two-particle correlations}
...
\section*{Appendix 7: Entanglement entropy, magnetisation and phase shift, at the QCP}
\addcontentsline{toc}{section}{\protect\numberline{}Appendix 7: Entanglement entropy, magnetisation and phase shift, at the QCP}
