...
\begin{equation}\begin{aligned}\label{VUb1}
	\sum_{k,q}Vc^\dagger_{d\uparrow}c_{q\uparrow} \frac{1}{\omega - H_D} U_b \sum_{\alpha,\alpha^\prime}\alpha\alpha^\prime c^\dagger_{q\alpha}c_{k\alpha} n_{q^\prime,\alpha^\prime} %= \sum_{k,\beta,q,i}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{2}\left(\frac{n_{d\bar\beta}}{\omega^1_i - E_1} + \frac{1 - n_{d\bar\beta}}{\omega^0_i - E_0}\right) U_b c^\dagger_{q\beta}c_{k\beta} n_{\bar q,\bar\beta}~,
\end{aligned}\end{equation}
where \(E_1 \) and \(E_0\) are excited state energies associated with the occupied and unoccupied configurations of \(d\bar\beta\), while \(\omega^{1/0}_0\) and \(\omega^{1/0}_1\) represent the energy associated with the initial and final states in the scattering process, for the particle and hole projectors of \(d\bar\beta\) respectively. When projected by \(n_{d\bar\beta}\), the initial and excited states have \(n_d = 1\), leading to \(E_1 = D/2 - U_b/2 - U/2 - J/4\) and \(\omega^1_0 = \omega - U/2\) while the final state has \(n_d = 2\) which gives \(\omega_1^1 = \omega\). When projected by \(1 - n_{d\bar\beta}\), we have \(n_d = 0\) in the initial and excited state resulting in \(E_0 = D/2 - U_b/2\) and \(\omega^0_0 = \omega\) while the final state has \(n_d=1\) leading to \(\omega^0_1 = \omega - U/2\). Combing them gives
\begin{equation}\begin{aligned}
	\sum_{k,\beta,q}VU_b c^\dagger_{d\beta}c_{k\beta} (1 - n_{q\beta}) n_{\bar q,\bar\beta}\frac{1}{2}\left[\frac{n_{d\bar\beta}}{\omega - D/2 + U_b/2 + J/4} + \frac{n_{d\bar\beta}}{\omega - D/2 + U_b/2 + J/4 + U/2}\right]\\
	+ \sum_{k,\beta,q}VU_b c^\dagger_{d\beta}c_{k\beta} (1 - n_{q\beta}) n_{\bar q,\bar\beta}\frac{1}{2}\left[\frac{1 - n_{d\bar\beta}}{\omega - D/2 + U_b/2} + \frac{1 - n_{d\bar\beta}}{\omega - D/2 + U_b/2 + U/2}\right]
\end{aligned}\end{equation}

Another process is constructed by removing an electron from the impurity site:
\begin{equation}\begin{aligned}\label{VUb2}
	\sum_{k,\beta,\bar q}Vc^\dagger_{\bar q\beta}c_{d\beta} \frac{1}{\omega - H_D} U_b c^\dagger_{k\beta}c_{\bar q\beta} n_{\bar q,\bar\beta} = \sum_{k,\beta,q,i}Vc^\dagger_{\bar q\beta}c_{d\beta} \frac{1}{2}\left(\frac{n_{d\bar\beta}}{\omega^1_i - E_1} + \frac{1 - n_{d\bar\beta}}{\omega^0_i - E_0}\right) U_b c^\dagger_{k\beta}c_{\bar q\beta} n_{\bar q,\bar\beta}~.
\end{aligned}\end{equation}
By identifying the various configurations, we again obtain the various energies: \(E_1 = D/2 - U_b/2, \omega^1_0 = \omega, \omega^1_1 = \omega - U/2\), and \(E_0 = D/2 - U_b/2 - U/2 - J/4, \omega^0_0 = \omega - U/2, \omega^0_1 = \omega\). Substituting these, we get
\begin{equation}\begin{aligned}
&-\sum_{k,\beta,q,i}VU_bc^\dagger_{k\beta}c_{d\beta} n_{\bar q\beta} n_{\bar q,\bar\beta}\frac{1}{2}\left[\frac{n_{d\bar\beta}}{\omega - D/2 + U_b/2} + \frac{n_{d\bar\beta}}{\omega - D/2 + U_b/2 - U/2}\right]\\
&-\sum_{k,\beta,q,i}VU_bc^\dagger_{k\beta}c_{d\beta} n_{\bar q\beta} n_{\bar q,\bar\beta}\frac{1}{2}\left[\frac{1 - n_{d\bar\beta}}{\omega - D/2 + U_b/2 + J/4} + \frac{1 - n_{d\bar\beta}}{\omega - D/2 + U_b/2 + U/2 + J/4}\right]
\end{aligned}\end{equation}

Another set of processes is generated by reversing the two operations. Corresponding to the first one, we have
\begin{equation}\begin{aligned}
	\sum_{k,\beta,q}U_b c^\dagger_{\bar q\beta}c_{k\beta} n_{\bar q,\bar\beta} \frac{1}{\omega - H_D}   Vc^\dagger_{d\beta}c_{\bar q\beta}~,
\end{aligned}\end{equation}
but this is just the hermitian conjugate of the process analysed in eq.~\ref{VUb2}, and we can write down the final result directly:
\begin{equation}\begin{aligned}
&-\sum_{k,\beta,q,i}VU_bc^\dagger_{d\beta}c_{k\beta} n_{\bar q\beta} n_{\bar q,\bar\beta}\frac{1}{2}\left[\frac{n_{d\bar\beta}}{\omega - D/2 + U_b/2} + \frac{n_{d\bar\beta}}{\omega - D/2 + U_b/2 - U/2}\right]\\
&-\sum_{k,\beta,q,i}VU_bc^\dagger_{d\beta}c_{k\beta} n_{\bar q\beta} n_{\bar q,\bar\beta}\frac{1}{2}\left[\frac{1 - n_{d\bar\beta}}{\omega - D/2 + U_b/2 + J/4} + \frac{1 - n_{d\bar\beta}}{\omega - D/2 + U_b/2 + U/2 + J/4}\right]~.
\end{aligned}\end{equation}
The final processes we have to consider is the flipped version of eq.~\ref{VUb2}:
\begin{equation}\begin{aligned}
	\sum_{k,\beta,q} U_b c^\dagger_{k\beta}c_{q\beta} n_{\bar q,\bar\beta} \frac{1}{\omega - H_D}Vc^\dagger_{q\beta}c_{d\beta}~,
\end{aligned}\end{equation}
but this is again the hermitian conjugate of eq.~\ref{VUb1}, so we get
\begin{equation}\begin{aligned}
	\sum_{k,\beta,q}VU_b c^\dagger_{k\beta}c_{d\beta} (1 - n_{q\beta}) n_{\bar q,\bar\beta}\frac{1}{2}\left[\frac{n_{d\bar\beta}}{\omega - D/2 + U_b/2 + J/4 + U/2} + \frac{n_{d\bar\beta}}{\omega - D/2 + U_b/2 + J/4}\right]\\
	+ \sum_{k,\beta,q}VU_b c^\dagger_{k\beta}c_{d\beta} (1 - n_{q\beta}) n_{\bar q,\bar\beta}\frac{1}{2}\left[\frac{1 - n_{d\bar\beta}}{\omega - D/2 + U_b/2 - U/2} + \frac{1 - n_{d\bar\beta}}{\omega - D/2 + U_b/2}\right]
\end{aligned}\end{equation}
