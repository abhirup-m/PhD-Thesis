...
\end{eqnarray}
The renormalisation in \(V\) from \(U_b\) arises through terms of \(V U_b\) and \(U_b V\) kind. One particular scattering process in this category involves adding an electron on the impurity site:
\begin{equation}\begin{aligned}\label{VUb1}
	\sum_{k,\beta,q}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{\omega - H_D} U_b c^\dagger_{q\beta}c_{k\beta} n_{\bar q,\bar\beta} = \sum_{k,\beta,q,i}Vc^\dagger_{d\beta}c_{q\beta} \frac{1}{2}\left(\frac{n_{d\bar\beta}}{\omega^1_i - E_1} + \frac{1 - n_{d\bar\beta}}{\omega^0_i - E_0}\right) U_b c^\dagger_{q\beta}c_{k\beta} n_{\bar q,\bar\beta}~,
\end{aligned}\end{equation}
