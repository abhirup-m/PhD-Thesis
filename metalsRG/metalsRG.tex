\chapter{\chapG}\label{sec:Metals}
...
One example of this is the MERA; it works by disentangling degrees of freedom in real space, creating a tensor network in the process~\cite{evenbly2011,reyes_2020,McMahon2020} and leading to a renormalisation in the entanglement. The renormalised entanglement is interpreted as the additional dimension~\cite{swingle2012a}.
...
\begin{equation}\begin{aligned}
	\mathcal{L} = \int \mathrm{d}x~\mathrm{d}y~\overline\psi(x,y)\left[\gamma^\mu(i\partial_\mu - eA_\mu) - m \right]\psi(x,y)~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\left[\gamma^\mu(i\partial_\mu - eA_\mu) - m\right]\psi^a(x,y) = 0~,
\end{aligned}\end{equation}
which is equivalent to the form
\begin{equation}\begin{aligned}
\int dx~ \overline{\psi^a}(x,y)\left[\gamma^\nu(i\partial_\nu + eA_\nu) + m\right]\left[\gamma^\mu(i\partial_\mu - eA_\mu) - m\right]\psi^a(x,y) = 0~.
\end{aligned}\end{equation}
...
\begin{equation}\label{DEdual}\begin{aligned}
	\sum_{m,n}\left(\overline{\psi^a}(k_x^m,y) e^{-ik_x^m x}\right) \left[\gamma^\nu(i\partial_\nu + eA_\nu) + m\right]\left[\gamma^\mu(i\partial_\mu - eA_\mu) - m\right]\left(e^{ik_x^n x}\psi^a(k_x^n,y)\right) = 0~.
\end{aligned}\end{equation}
Evaluating the right operation gives
\begin{equation}\begin{aligned}
	\left[\gamma^\mu(i\partial_\mu - eA_\mu) - m\right]\left(e^{ik_x^n x}\psi^a(k_x^n,y)\right) = e^{ik_x^n x}\left[\gamma^\mu(i\partial_\mu - eA_\mu) - \left( \gamma^x (k_x^n + eA_x) + m\right) \right]\psi^a~.
\end{aligned}\end{equation}
Similarly, the left operation gives
\begin{equation}\begin{aligned}
	\left(\overline{\psi^a}(k_x^m,y) e^{-ik_x^m x}\right) \left[\gamma^\nu(i\partial_\nu + eA_\nu) + m\right] &= \left[\left[\gamma^\nu(i\partial_\nu - eA_\nu) + m\right]\left(e^{ik_x^m x}\gamma_0\psi^a\right)\right]^\dagger \\
							      &= \overline{\psi^a} e^{-ik_x^m x}\left[\gamma_\nu(i\partial^\nu + eA_\nu) - \left( \gamma^x (k_x^n + eA_x) - m \right) \right]~.
\end{aligned}\end{equation}
Substituting these into the equation eq.\eqref{DEdual} gives
\begin{equation}\begin{aligned}
	\sum_{n}\int\mathrm{d}y ~ \overline{\psi^a}(k_x^n,y) \left[-(i\partial_\mu + eA_\mu)(i\partial_\mu - eA_\mu) + (k_x^n + eA_x)^2 + m^2\right]\psi^a(k_x^n,y)= 0~,
\end{aligned}\end{equation}
where we have used \(\int \mathrm{d}x ~e^{ix\left( k_x^m - k_x^n \right) } = \delta_{m,n}\).
In terms of these dual fields, the Lagrangian can be shown to be equivalent to a tower of massive \(1+1-\)dimensional fermions (see Fig.~\ref{dim-red}):
\begin{equation}\begin{aligned}
\mathcal{L} = \sum_{k_x^n} \int \mathrm{d}y~\overline\psi(k_x^n,y)\left(\gamma^\mu (i\partial_\mu - eA_\mu) - M(n,\phi)\right) \psi(k_x^n,y)~,
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\sum_{n=-\infty}^\infty \ln|n+\phi| = \sum_{n=-\infty}^\infty \left[\ln|n+\phi| + \ln|n+\left( 1-\phi \right) |\right] = -\frac{\partial{\zeta(\phi)}}{\partial{\phi}} - \frac{\partial{\zeta(1-\phi)}}{\partial{\phi}}~,
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\label{area-term}
\lim_{N \to \infty} \sum_{n=-N,\atop{-N+t_{z}(i),\ldots}}^N c\ln \frac{L_x}{2\pi\epsilon} = \lim_{N \to \infty} \sum_{n^\prime=-\frac{N}{t_{z}(i)},\atop{-\frac{N}{t_{z}(i)}+1,\ldots}}^{\frac{N}{t_{z}(i)}} c\ln \frac{L_x}{2\pi\epsilon} = t^{-1}_{z}(i) \lim_{N \to \infty} \sum_{n=-N}^N c\ln \frac{L_x}{2\pi\epsilon} \rightarrow c f_{z}(i)\frac{\alpha L_x}{\epsilon}~.
\end{aligned}\end{equation}
Similarly, for the second term, we have
\begin{equation}\begin{aligned}
	\lim_{N \to \infty} \sum_{n=-N,\atop{-N+t_{z}(i),\ldots}}^N \ln|n+\phi| = \lim_{N \to \infty} \sum_{n^\prime=-\frac{N}{t_{z}(i)},\atop{-\frac{N}{t_{z}(i)}+1,\ldots}}^{\frac{N}{t_{z}(i)}} \ln|n^\prime t_{z}(i) + \phi| = \lim_{N \to \infty} \sum_{n^\prime=-\frac{N}{t_{z}(i)},\atop{-\frac{N}{t_{z}(i)}+1,\ldots}}^{\frac{N}{t_{z}(i)}} \ln \left(f_{z}(i)|n^\prime + f_{z}(i)\phi|\right) \\
...
\begin{equation}\begin{aligned}\label{mutinfo}
	I^2(i,j) &\equiv S_{z}(i) + S_{j,z} - S_{i \cup j,z} = \theta(-z) S_{\text{min}(i,j),z} + \theta(z) S_{\text{max}(i,j),z}~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	I^3 = S_{\mathcal{A}_{z}(0)} + S_{\mathcal{A}_{z}(1)} + S_{\mathcal{A}_{z}(2)} - S_{\mathcal{A}_{z}(0)\cup \mathcal{A}_{z}(1)} - S_{\mathcal{A}_{z}(1)\cup \mathcal{A}_{z}(2)} - S_{\mathcal{A}_{z}(2)\cup \mathcal{A}_{z}(0)} + S_{\mathcal{A}_{z}(0)\cup \mathcal{A}_{z}(1) \cup \mathcal{A}_{z}(2)}~.
\end{aligned}\end{equation}
For our choice of subsystems, the \(g-\)partite information can be shown to be equal to the EE of the {\it intersection set} of the concerned \(g-\)sets (details in \hyperref[g-partite info]{Appendix A}). For the case of \(z > 0\), this takes the form
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	P_{j,z} = \sum_{k_x^1=0,1}\sum_{k_x^2=0,1}\ldots\sum_{k_x^N=0,1} \ket{\hat n_{k_x^1} \hat n_{k_x^2}\ldots}\bra{\hat n_{k_x^1} \hat n_{k_x^2}\ldots},\quad k_x^j \in \mathcal{A}_{j,z}~,
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\label{info-dist2}
	d_z(i,j) = \theta(-z) \ln \frac{S_\text{max}}{S_{z}(i)} + \theta(z) \ln \frac{S_\text{max}}{S_{j,z}} = \theta(z)j^z\ln 2 + \theta(-z) i^z\ln 2~.
\end{aligned}\end{equation}
\begin{figure}
	\centering
...
\begin{equation}\begin{aligned}
	g_z(n,j) &= \ln \frac{M_{n+1,\phi}(j) - M_{n,\phi}(j)}{2\pi/L_x} \\
...
\begin{equation}\begin{aligned}
	Q_g(\phi) = f_{g,z}\left[\frac{1}{\sqrt 2} - \frac{1}{2}e^{-\frac{1}{c}\left(I^g_{\left\{ \mathcal{A}_{z}(i) \right\}}(\phi) - I^g_{\left\{ \mathcal{A}_{z}(i) \right\}}(\phi^{(0)}f_{g,z})\right)}\right] = f_{g,z}\left[\frac{1}{\sqrt 2} - \big|\sin \left(\pi\phi f_{g,z}\right)\big|\right] ~,
\end{aligned}\end{equation}
...
The evaluation of the integral eq.\eqref{pole-1} is shown in \hyperref[contour-integral]{Appendix~B}.
...
\begin{equation}\begin{aligned}
	\mathcal{C}_j(f_{g,z}\phi) \equiv \mathcal{C}^g_j(\phi) = \bigg\{\phi:\phi = t_{g,z}\left( j - \frac{1}{4} + \frac{e^{i\theta}}{8} \right),~\theta \in \left[0, -2\pi f_j\right]\bigg\}~.
\end{aligned}\end{equation}
...
\section*{Appendix A: Evaluation of multi-partite information}\label{g-partite info}
...
\section*{Appendix B: Evaluation of the contour integral that counts poles}\label{contour-integral}
