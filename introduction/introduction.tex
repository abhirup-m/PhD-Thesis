...
\end{equation}
mean-field theory proceeds by replacing the interaction between a given spin and its neighbors by an effective static field proportional to the average magnetization. Writing
\(
S_j = \langle S_j \rangle + \delta S_j
\)
and neglecting quadratic fluctuations and constant parts, one obtains a mean-field Hamiltonian
\begin{equation}\begin{aligned}
...
\begin{equation}
S_{\text{imp}} = -\int_0^\beta d\tau d\tau'\,c^\dagger(\tau)\,G_0^{-1}(\tau-\tau')\,c(\tau') + U \int_0^\beta d\tau\, n_\uparrow(\tau) n_\downarrow(\tau).
\end{equation}
Such an impurity problem is solvable, and the impurity Green's function is given by
\begin{equation}
G_{\text{imp}}(\omega) =
\frac{1}{G_0^{-1}(\omega) - \Sigma(\omega)},
\label{eq:impG}
\end{equation}
...
\label{eq:dmftSC}
\end{equation}
Combining Eqs.~\eqref{eq:localG} and \eqref{eq:impG}, one obtains a self-consistency equation that determines the Weiss field \(\mathcal{G}_0(\omega)\),
\begin{equation}
\mathcal{G}_0^{-1}(\omega)
= \Sigma(\omega) + G_{\text{loc}}^{-1}(\omega).
\end{equation}

Together with the definition of \(G_{\text{loc}}(\omega)\) as a functional of \(\Sigma(\omega)\), this equation closes the DMFT loop. The interacting lattice problem is thus reduced to the self-consistent solution of a quantum impurity model, capturing all local dynamical correlations exactly while neglecting nonlocal ones \cite{Georges1996,Kotliar2004}.
