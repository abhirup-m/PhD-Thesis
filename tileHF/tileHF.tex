...
\begin{equation}\begin{aligned}
	H_f = -\frac{U_f}{2} (n_{f,\uparrow} - n_{f,\downarrow})^2 + \sum_{Z \in \text{NN}}&\left[-V\sum_{\sigma}\left(c^\dagger_{f,\sigma} c_{Z,\sigma,f} + \text{h.c.}\right) + \frac{J}{4}\sum_{\alpha,\beta}{\bf S}_f \cdot {\boldsymbol{\sigma}}_{\alpha\beta}c^\dagger_{Z,\alpha,f}c_{Z,\beta,f} \right.\\
		 &\left.- \frac{W_f}{4} \left(n_{Z,\uparrow,f} - n_{Z,\downarrow,f}\right)^2\right] ~,\\
	H_d = -\eta_d n_{d} - \frac{U_d}{2}(n_{d,\uparrow} - n_{d,\downarrow})^2 + \sum_{Z \in \text{NN}}&\left[-V\sum_{\sigma}\left(c^\dagger_{d,\sigma} c_{Z,\sigma,d} + \text{h.c.}\right) + \frac{J}{4}\sum_{\alpha,\beta}{\bf S}_d \cdot {\boldsymbol{\sigma}}_{\alpha\beta}c^\dagger_{Z,\alpha,d}c_{Z,\beta,d}\right] ~,
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	H_\text{aux} =& \sum_{{\bf k},\sigma,\alpha}\epsilon_{{\bf k},\alpha}n_{{\bf k},\sigma,\alpha} + \sum_\alpha\sum_{Z \in \text{NN}}\left[\frac{1}{4}\sum_{\sigma,\sigma^\prime}\left(J_\alpha {\bf S}_\alpha + K_{\bar\alpha}{\bf S}_{\bar\alpha} \right)\cdot {\boldsymbol{\sigma}}_{\sigma\sigma^\prime}c^\dagger_{Z,\sigma,\alpha}c_{Z,\sigma^\prime,\alpha} \right.\\
...
\begin{equation}\begin{aligned}
	P_\text{dir} = \sum_{\alpha, k, q} J_\alpha {\bf S}_\alpha \cdot {\bf \sigma}_{\sigma_1,\sigma_2} c^\dagger_{q,\sigma_1,f}c_{k,\sigma_2,f}~,\\
	P_\text{ind} = \sum_{\alpha, k, q} K_{\bar\alpha} {\bf S}_{\bar\alpha} \cdot {\bf \sigma}_{\sigma_1,\sigma_2} c^\dagger_{q,\sigma_1, f}c_{k,\sigma_2, f}~.
\end{aligned}\end{equation}

\paragraph{Hole sector}
These involve excited states below the Fermi energy:
\begin{equation}\begin{aligned}
	H_\text{dir} = \sum_{\alpha, k, q} J_\alpha {\bf S}_\alpha \cdot {\bf \sigma}_{\sigma_1,\sigma_2} c^\dagger_{k,\sigma_1, f}c_{q,\sigma_2, f}~,\\
	H_\text{ind} = \sum_{\alpha, k, q} K_{\bar\alpha} {\bf S}_{\bar\alpha} \cdot {\bf \sigma}_{\sigma_1,\sigma_2} c^\dagger_{k,\sigma_1, f}c_{q,\sigma_2, f}~.
\end{aligned}\end{equation}
\paragraph{Mixed sector}
These involve simultaneous particle and hole excitations:
\begin{equation}\begin{aligned}
	M_\text{dir} = \sum_{\alpha, q, \bar q} J_\alpha {\bf S}_\alpha \cdot {\bf \sigma}_{\sigma_1,\sigma_2} c^\dagger_{q,\sigma_1, f}c_{\bar q,\sigma_2, f}~,\\
	M_\text{ind} = \sum_{\alpha, q, \bar q} K_{\bar\alpha} {\bf S}_{\bar\alpha} \cdot {\bf \sigma}_{\sigma_1,\sigma_2} c^\dagger_{q,\sigma_1, f}c_{\bar q,\sigma_2}~.
\end{aligned}\end{equation}

\subsection{Renormalisation of direct Kondo coupling \(J\)}
We already have the renormalisation group equations for the couplings \(J_\alpha\) in the case of \(J=K=0\):
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
We now simplify the second term. Taking into account the other spin operators on either side of \(G_f({\bf q})\) in the full scattering process, we have
\begin{equation}\begin{aligned}
	\sum_{a,b,c} \sum_{\alpha,\beta} \sum_{\gamma,\delta} S_f^a \sigma^a_{\alpha,\beta} S_f^b S_d^b S_f^c \sigma^c_{\gamma,\delta}\left[\delta_{\beta,\gamma}c^\dagger_{k,\alpha,f}c_{q,\beta,f} c^\dagger_{q,\gamma,f}c_{k^\prime,\delta,f} + \delta_{\alpha,\delta}c^\dagger_{\bar q,\alpha,f}c_{k^\prime,\beta,f} c^\dagger_{k,\gamma,f}c_{\bar q,\delta,f}\right]~.
\end{aligned}\end{equation}
Integrating out the UV states using \(n_{\bar q\beta} = 1 - n_{q\beta} = 1\), we get
\begin{equation}\begin{aligned}
	\sum_{a,b,c} \sum_{\alpha,\beta,\gamma} S_f^a S_f^b S_d^b S_f^c \left[\sigma^a_{\alpha,\beta} \sigma^c_{\beta,\gamma}c^\dagger_{k,\alpha,f} c_{k^\prime,\gamma,f} + \sigma^a_{\alpha,\beta} \sigma^c_{\gamma,\alpha}c_{k^\prime,\beta,f} c^\dagger_{k,\gamma,f}\right]~.
\end{aligned}\end{equation}
Using \(\sum_\beta \sigma^a_{\alpha,\beta} \sigma^c_{\beta,\gamma} = (\sigma^a \sigma^c)_{\alpha,\gamma} = \delta_{ac} + i \epsilon_{acm} \sigma^m_{\alpha,\gamma}\) in the first term and \(\sum_\alpha \sigma^c_{\gamma,\alpha} \sigma^a_{\alpha,\beta} = (\sigma^c \sigma^a)_{\gamma,\beta} = \delta_{ca} - i\epsilon_{acm}\sigma^m_{\gamma,\beta}\) in the second term, relabelling \(\beta \to \gamma\) and \(\gamma \to \alpha\) there and using \(c_{k^\prime,\gamma} c^\dagger_{k,\alpha} = \delta_{k,k^\prime}\delta_{\alpha,\gamma} - c^\dagger_{k,\alpha} c_{k^\prime,\gamma}\), we get
\begin{equation}\begin{aligned}
	\sum_{a,b,c} \sum_{\alpha,\gamma} S_f^a S_f^b S_d^b S_f^c \left[\delta_{a,c}\delta_{k,k^\prime}\delta_{\alpha,\gamma} + 2i \epsilon_{acm} \sigma^m_{\alpha,\gamma}c^\dagger_{k,\alpha,f} c_{k^\prime,\gamma,f}\right]~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	&\sum_{a,b,c} \sum_{\alpha,\gamma} S_f^a S_f^b S_d^b S_f^c \left[\delta_{a,c}\delta_{k,k^\prime}\delta_{\alpha,\gamma} + 2i \epsilon_{acm} \sigma^m_{\alpha,\gamma}c^\dagger_{k,\alpha,f} c_{k^\prime,\gamma,f}\right]\\
	&= -\frac{1}{2}\sum_{b} S_f^b S_d^b \delta_{k,k^\prime} + \frac{1}{2}\sum_{b} \sum_{\alpha,\gamma} S_d^b \sigma^b_{\alpha,\gamma}c^\dagger_{k,\alpha,f} c_{k^\prime,\gamma,f}\\
	&= -\frac{1}{2}{\bf S}_f \cdot {\bf S}_d \delta_{k,k^\prime} + \frac{1}{2} \sum_{\alpha,\gamma} {\bf S}_d {\bf \sigma}_{\alpha,\gamma}c^\dagger_{k,\alpha,f} c_{k^\prime,\gamma,f}\\
\end{aligned}\end{equation}
