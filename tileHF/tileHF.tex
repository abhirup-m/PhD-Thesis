...
\begin{equation}\begin{aligned}
	G(\bm k,d;\omega) =& \sum_{\alpha=\pm}C_{\alpha, \bm k}\left[\mathcal{G}^>(\mathcal{T}_{\bm k,\alpha,d}; \omega - \epsilon_k + \eta_d/2 + \alpha\Delta) + \mathcal{G}^<(\mathcal{T}_{\bm k,\alpha,d}; \omega + \epsilon_k - \eta_d/2 - \alpha\Delta)\right]~.
\end{aligned}\end{equation}

Following eq.~\ref{localTiledGreenF}, we can similarly write down an expression the local lattice Greens function, in real-space:
\begin{equation}\begin{aligned}
	G&_\text{loc}(\omega)\\
	=&~ \mathcal{G}(d\sigma; \omega) \\
						 &+ \sum_{\bm k, \alpha=\pm} C_{\alpha, \bm k}\left[\mathcal{G}^>(\mathcal{T}_{\bm k,\alpha,d},d\sigma; \omega - \epsilon_k + \eta_d/2 + \alpha\Delta) + \mathcal{G}^<(\mathcal{T}_{\bm k,\alpha,d},d\sigma; \omega + \epsilon_k - \eta_d/2 - \alpha\Delta)\right]\\
						   &+\sum_{\bm k} \mathcal{G}(d\sigma,\mathcal{T}_{\bm k,\sigma}; \omega)  \\
						   &+ \sum_{\bm k, \bm q, \alpha=\pm} C_{\alpha, \bm k}\left[\mathcal{G}^>(\mathcal{T}_{\bm k,\alpha,d}, \mathcal{T}_{\bm q,\sigma, d}; \omega - \epsilon_k + \eta_d/2 + \alpha\Delta) + \mathcal{G}^<(\mathcal{T}_{\bm k,\alpha,d}, \mathcal{T}_{\bm q,\sigma, d}; \omega + \epsilon_k - \eta_d/2 - \alpha\Delta)\right]\\
\end{aligned}\end{equation}
