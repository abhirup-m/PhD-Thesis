...
\end{aligned}\end{equation}
Tiling the impurity model therefore leads to a bilayer extended Hubbard model, where the tilde indicates that 
...
\begin{equation}\begin{aligned}
	G(\bm k,\sigma,\alpha;\omega) = \theta(t)\mathcal{G}^>(\mathcal{T}_{\bm k,\sigma,\alpha}; \omega - \epsilon_{\bm k,\alpha}) - \mathcal{G}^<(\mathcal{T}_{\bm k,\sigma,\alpha}; \omega + \epsilon_{\bm k,\alpha})~,
\end{aligned}\end{equation}
where \(\alpha=f,d\) denotes the layer in which the Greens function is being calculated. The frequency shift \(\epsilon_{\bm k,\alpha}\) was defined as the expectation value \(\braket{n_{k,\sigma,\alpha}=1|H_\text{1p}|n_{k,\sigma,\alpha}=0}\), where \(H_\text{1p}\) is the non-interacting part of the lattice Hamiltonian:
\begin{equation}\begin{aligned}
