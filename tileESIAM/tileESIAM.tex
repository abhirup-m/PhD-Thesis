...
\section{Effective Hamiltonian for excitons in the Mott insulator}
...
\begin{equation}\begin{aligned}
	H_0 = -\frac{U}{2}\sum_i (n_{i \uparrow} - n_{i \downarrow})^2, \quad H_1 = -t\sum_{\langle i,j \rangle , \sigma}(c^\dagger_{i,\sigma}c_{j,\sigma} + \text{h.c.})~.
\end{aligned}\end{equation}
The zeroth order ground state is of course half-filled, each site being singly-occupied:
\begin{equation}\begin{aligned}
	\ket{\Psi_0} = \otimes_i \ket{\sigma_i}~,
\end{aligned}\end{equation}
where \(sigma_i\) can be \(\pm 1\) and indicates that each site hosts a local moment that can be either up or down, but doubles or holes are prohibited. This also reflects the large degeneracy (\(\sim 2^N\)) associated with such a state.

We first note that a non-zero hopping probability \(t\) indicates that the ground state cannot be just \(\ket{\Psi_0}\), simply because \(\ket{\Psi_0}\) is not an eigenstate of \(H_1\). In order to consider hopping processes that start from the half-filled ground state, we define a projector \(P_H = \otimes_i P_H(i) = \otimes_i (n_{i \uparrow} + n_{i \downarrow} - 2n_{i \uparrow} n_{i \downarrow})\) that annihilates the hole or double state at any site. The only allowed processes are therefore
\begin{equation}\begin{aligned}
	H_\text{eff} = H_1 P_H = -t\sum_{\langle i,j \rangle , \sigma}(c^\dagger_{i,\sigma}P_H(i) c_{j,\sigma}P_H(j) + \text{h.c.})~.
\end{aligned}\end{equation}
The action of the projector on any local operator is easy to calculate:
\begin{equation}\begin{aligned}
	c_{j,\sigma}P_H(j) &=  c_{j\sigma} (n_{j \sigma} + n_{j \bar\sigma} - 2n_{j \sigma} n_{j \bar \sigma}) =  c_{j \sigma}(1 - n_{j\bar\sigma})~,\\
	c^\dagger_{i,\sigma}P_H(i) &=  c^\dagger_{i,\sigma} (n_{j \sigma} + n_{j \bar\sigma} - 2n_{j \sigma} n_{j \bar \sigma}) =  c^\dagger_{i \sigma}n_{i\bar\sigma}~,
\end{aligned}\end{equation}
The effective Hamiltonian for such hopping processes from the correlated ground state is therefore
\begin{equation}\begin{aligned}
	H_\text{eff} = -t\sum_{\langle i,j \rangle ,\sigma}(c^\dagger_{i \sigma}c_{j \sigma}n_{i\bar\sigma}(1 - n_{j\bar\sigma}) + \text{h.c.})~.
\end{aligned}\end{equation}
In order to identify the propagating degrees of freedom, we define an exciton creation operator \(b^\dagger\):
\begin{equation}\begin{aligned}
	b^\dagger_{i, \delta} = B\sum_\sigma c^\dagger_{i \sigma}c_{i + \delta, \sigma}n_{i\bar\sigma}(1 - n_{i + \delta,\bar\sigma})~,
\end{aligned}\end{equation}
%%% TODO check the algebra
where \(\delta\) is a primitive lattice vector. This operator creates an exciton: a bound holon-doublon pair between the sites \(i\) and \(i + \delta\). In terms of these operators, the Hamiltonian becomes
\begin{equation}\begin{aligned}
	H_\text{eff} = -tB\sum_{i,\delta}(b^\dagger_{i,\delta} + \text{h.c.})~.
\end{aligned}\end{equation}
