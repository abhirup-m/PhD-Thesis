...
\end{aligned}\end{equation}
We recognise the two parts of the lattice model Greens function as a retarded off-diagonal Greens functions calculated within the auxiliary model at \(\bm r_d\), but at shifted frequencies \(\omega - \epsilon_\nu\):
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	G({\bm r}\sigma; t) = -i\theta(t) \sum_{\bm r_d}\braket{\Psi_\text{gs} | \{ c_{{\bm r_d + \bm r},\sigma}(t), c^\dagger_{{\bm r_d},\sigma} \} | \Psi_\text{gs}}~.
\end{aligned}\end{equation}
Similar to the \(k-\)space Greens functions, express the groundstates as a superposition of auxiliary model Greens functions. We take the first term in the anticommutator and proceed to simplify it:
...
\begin{equation}\begin{aligned}
	\sum_{\bm r_d}\braket{\Psi_\text{gs} | c_{{\bm r_d},\sigma}(t) c^\dagger_{{\bm r_d},\sigma} | \Psi_\text{gs}} &= \braket{\psi_\text{gs}({\bm r_d}) |  c_{{\bm r_d},\sigma}(t) c^\dagger_{{\bm r_d},\sigma} | \psi_\text{gs}({\bm r_d})}\\
																													  &+ \sum_{\bm r \neq r_d, n}F_n(\bm r - \bm r_d)\braket{\psi_\text{gs}({\bm r_d}) | \mathcal{T}_{\bm r,\sigma}(t) | \psi_{n}(\bm r_d)} \braket{\psi_{n}(\bm r_d) | c^\dagger_{{\bm r_d},\sigma} | \psi_\text{gs}({\bm r_d})}\\
																													  &+ \sum_{\bm r \neq r_d, n}{F_n(\bm r - \bm r_d)}^*\braket{\psi_\text{gs}({\bm r_d}) | c_{\bm r_d,\sigma}(t) | \psi_{n}(\bm r_d)} \braket{\psi_{n}(\bm r_d) | \mathcal{T}^\dagger_{\bm r,\sigma} | \psi_\text{gs}({\bm r_d})}\\
...
We next treat the time-evolution of the operators exactly as in the \(k-\)space Greens function. Without repeating the details, we state the final result here, for the frequency-domain real-space local Greens function:
\begin{equation}\begin{aligned}\label{localTiledGreenF}
	G_\text{loc}(\omega) =& \sum_\nu C_{\nu,\bm r_d} \mathcal{G}(\nu(r_d)\sigma, c^\dagger_{r_d,\sigma}; \omega - \epsilon_\nu^\text{loc}) \\
						 &+ \sum_{\bm r\neq r_d, \nu} C_{\bm r, \nu}F_n(\bm r - \bm r_d)\mathcal{G}(\mathcal{T}_{\nu(\bm r)\sigma},c^\dagger_{r_d\sigma};\omega - \epsilon_\nu^\text{loc})\\
						 &+ \sum_{\bm r\neq r_d, \nu} C_{\bm r_d, \nu} F_n(\bm r - \bm r_d)^*\mathcal{G}(\nu(\bm r_d)\sigma,\mathcal{T}^\dagger_{\bm r\sigma}; \omega - \epsilon_\nu^\text{loc})\\
						 &+ \sum_{\bm r_1,\bm r_2 \neq r_d, \nu} C_{\bm r_1, \nu_1} F_n(\bm r_1 - \bm r_2)^* \mathcal{G}(\mathcal{T}_{\nu(\bm r_1)\sigma},\mathcal{T}^\dagger_{\bm r_2\sigma}; \omega - \epsilon_\nu^\text{loc}) ~,
\end{aligned}\end{equation}
where [NEED TO EXPLAIN ALL THE TERMS!]. % TODO
