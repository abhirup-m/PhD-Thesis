...
\subsection{Periodisation of Auxiliary Model Into Extended Hubbard Model}
...
\begin{equation}\begin{aligned}\label{clusterHamiltonian}
	H_\text{clust}(\bm r_d) =& -\sum_{{\bm k},\sigma} e^{i \bm k \cdot \bm r_d } V({\bm k})(c^\dagger_{{\bm r}_d,\sigma} c_{{\bm k},\sigma} + \text{h.c.}) + \sum_{\bm k_1, \bm k_2} e^{i (\bm k_1 - \bm k_2) \cdot \bm r_d }~J_K({\bm k_1, \bm k_2}) {\bm S}_{\bm r_d} \cdot {\bm S}_{\bm k_1, \bm k_2} + \varepsilon_d \sum_\sigma n_{\bm r_d,\sigma}\\
...
\subsection{One-particle \(k-\)space Greens functions}
...
\end{aligned}\end{equation}
To make the problem tractable, we treat the effect of \(H_\text{1p}\) at the level of an expectation value and note that the effect of the transition operator is to create a hole of momentum \(\bm k\) (eq.~\ref{tmatrix}). This leads to an excitation cost \(\braket{n_k=0|H_\text{1p}|n_k=1} = -\epsilon_k\). Moreover, \(\ket{\psi_n(\bm r_d)}\) and \(\ket{\psi_\text{gs}(\bm r_d)}\) are eigenstates of \(H_\text{clust}(\bm r_d)\) with energies \(E_n\) and \(E_0\). This leads to the following simplified expression:
\begin{equation}\begin{aligned}
	\braket{\psi_\text{gs}({\bm r_d}) | \mathcal{T}_{{\bm k},\sigma}(t) | \psi_{n}({\bm r_d})} = e^{-i(E_n - E_0 + \epsilon_k)t} \braket{\psi_\text{gs}({\bm r_d}) | \mathcal{T}_{{\bm k},\sigma} | \psi_{n}({\bm r_d})}~.
\end{aligned}\end{equation}
Treating the other overlap in eq.~\ref{intermediate} in an identical fashion gives
\begin{equation}\begin{aligned}
	\braket{\psi_{n}({\bm r_d}) c^\dagger_{{\bm k}\sigma} | \psi_\text{gs}({\bm r_d})} = \braket{\psi_n({\bm r_d}) | \mathcal{T}_{{\bm k},\sigma}^\dagger | \psi_\text{gs}({\bm r_d})}~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	...
	&= -i\theta(t) \sum_n e^{-i(E_n - E_0 + \epsilon_k)t} \braket{\psi_\text{gs}({\bm r_d}) | \mathcal{T}_{{\bm k},\sigma} | \psi_{n}({\bm r_d})}\braket{\psi_n({\bm r_d}) | \mathcal{T}_{{\bm k},\sigma}^\dagger | \psi_\text{gs}({\bm r_d})}~.
\end{aligned}\end{equation}
Working through the other term with the same strategy, we get the full expression:
\begin{equation}\begin{aligned}
	-i&\theta(t) \braket{\Psi_\text{gs} | \{c_{{\bm k}\sigma}(t), c^\dagger_{{\bm k}\sigma}\} | \Psi_\text{gs}} \\
	=& -i\theta(t) \sum_n e^{-i(E_n - E_0 + \epsilon_k)t} \braket{\psi_\text{gs}({\bm r_d}) | \mathcal{T}_{{\bm k},\sigma} | \psi_{n}({\bm r_d})}\braket{\psi_n({\bm r_d}) | \mathcal{T}_{{\bm k},\sigma}^\dagger | \psi_\text{gs}({\bm r_d})}\\
	& -i\theta(t) \sum_n e^{i(E_n - E_0 - \epsilon_k)t} \braket{\psi_\text{gs}({\bm r_d}) | \mathcal{T}^\dagger_{{\bm k},\sigma} | \psi_{n}({\bm r_d})}\braket{\psi_n({\bm r_d}) | \mathcal{T}_{{\bm k}\sigma} | \psi_\text{gs}({\bm r_d})}~.
\end{aligned}\end{equation}
Changing from time to frequency domain using \(g(\omega) = \int~dt~ e^{i\omega T}f(t)\) gives
\begin{equation}\begin{aligned}
	G_{\bm k}(\omega) =  \sum_n &\left[\frac{\braket{\psi_\text{gs}({\bm r_d}) | \mathcal{T}_{{\bm k}\sigma} | \psi_{n}({\bm r_d})}\braket{\psi_n({\bm r_d}) | \mathcal{T}_{{\bm k}\sigma}^\dagger | \psi_\text{gs}({\bm r_d})}}{\omega - (E_n - E_0 + \epsilon_{\bm k})}\right.\\
					   &- \left.\frac{\braket{\psi_\text{gs}({\bm r_d}) | \mathcal{T}^\dagger_{{\bm k},\sigma} | \psi_{n}({\bm r_d})}\braket{\psi_n({\bm r_d}) | \mathcal{T}_{{\bm k}\sigma} | \psi_\text{gs}({\bm r_d})}}{\omega + (E_n - E_0 - \epsilon_{\bm k})}\right]~.
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	G_{\bm k}(\omega) = \theta(t)\mathcal{G}^>(\mathcal{T}_{\bm k,\sigma}; \omega - \epsilon_{\bm k}) - \mathcal{G}^<(\mathcal{T}_{\bm k,\sigma}; \omega + \epsilon_{\bm k})~.
\end{aligned}\end{equation}
...
% \begin{equation}\begin{aligned}
% 	\braket{\Psi_\text{gs} | c_{{\bm K}\sigma}(t) c^\dagger_{{\bm K}\sigma} | \Psi_\text{gs}} = \frac{1}{2N}\sum_n\sum_{\vec r,\vec r^\prime,\vec \Delta^\prime} \bra{\psi_{0}(\vec r)} c_{{\bm K}\sigma}(t) \left[ \ket{\chi_n^+(\vec r^\prime,\vec\Delta^\prime)}\bra{\chi_n^+(\vec r^\prime,\vec\Delta^\prime)} \right.\\
% 	\left.- \ket{\chi_n^-(\vec r^\prime,\vec\Delta^\prime)}\bra{\chi_n^-(\vec r^\prime,\vec\Delta^\prime)}\right] c^\dagger_{{\bm K}\sigma} \ket{\psi_{0}(\vec r)}~.
% \end{aligned}\end{equation}
%
